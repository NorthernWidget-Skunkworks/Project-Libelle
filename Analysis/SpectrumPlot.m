...
title('Project Libelle Sub-sensor Spectral Response');
...
subplot(2,1,2);
Line1 = plot(SamplePoints, Libelle);
hold on
...
legend('Project Libelle', 'Kipp and Zonen CMP3', 'Apogee SP110', 'Solar Black Body');
ylabel('Spectral Sensitivity');
xlabel('Wavelength [nm]');
...
figure(3)
plot(SamplePoints_Long./1000, BlackBody_Earth)
hold on
plot(SamplePoints_Long./1000, IR_Long)
% plot(SamplePoints_Long, KippAndZonen_Long)
axis([2.5, 35, 0, 1.2]);
title('Long Wave Spectral Response');
legend('Project Libelle', 'Earth Black Body');
ylabel('Spectral Sensitivity');
...
axis([250, 2000, 0, 1.2]);
xtickangle(45);
title('Project Libelle Spectral Sample Points');
ylabel('Spectral Sensitivity');
xlabel('Wavelength [nm]');
